...
import { Factory, Globe2, Layers3, MessageCircle, PackageCheck, Ruler, ScanSearch, Scissors, Shirt, UserRound } from "lucide-react";
...
function Stats() {
  ...
  return (
    <div ref={ref} className="mx-auto grid max-w-4xl grid-cols-1 gap-8 px-6 py-12 text-center sm:grid-cols-3">
      {items.map((it, idx) => (
        <div key={idx}>
  ...
  return (
    <section className="home-services py-12">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <h3 className="mt-3 text-center font-serif text-2xl text-stone-700">From Concept to Collection, Crafted with Precision</h3>
    ...
    <section id="admin-contact" className="admin-contact-section scroll-mt-20">
      <div className="mx-auto max-w-6xl px-5 py-14 sm:px-6 sm:py-16">
        <div className="text-center">
          ...
          <h2 className="mt-3 font-serif text-3xl">Choose an Admin to Assist You</h2>
          <p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-stone-500">Contact our team via WhatsApp to discuss your service, production, or brand's creative needs.</p>
        </div>
        <div className="admin-contact-grid mt-9">
          {admins.map((admin, index) => (
            <article className="admin-contact-card" key={admin.name}>
              <span className="admin-card-number">{String(index + 1).padStart(2, "0")}</span>
              ...
              <a href={`https://wa.me/${admin.number}?text=Halo%20Admin%20${admin.name},%20saya%20ingin%20konsultasi%20tentang%20layanan%20Aireta.`} target="_blank" rel="noreferrer" className="admin-whatsapp-button">
                <MessageCircle size={16} strokeWidth={1.5} /> CHAT WHATSAPP <span>→</span>
              </a>
            </article>
          ))}
        </div>
        <p className="mt-5 text-center text-[10px] tracking-[0.08em] text-stone-400">AVAILABLE MONDAY—SATURDAY · 09:00—17:00 WIB</p>
      </div>
    </section>
  );
}
...
function Portfolio() {
  ...
  const labels = ["EVENING WEAR", "MODEST COLLECTION", "RESORT WEAR", "KIDS WEAR", "FASHION SHOW"];
  return (
    <section className="max-w-6xl mx-auto px-6 py-10">
      <div className="flex items-end justify-between gap-5">
        <div>
          <p className="section-kicker">FEATURED PROJECTS</p>
          <h3 className="mt-2 text-2xl font-serif">From Concept to Collection</h3>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-4 mt-6">
        {imgs.map((src, i) => (
          <div key={i} className="portfolio-card relative rounded overflow-hidden h-72 sm:h-64 md:h-48">
...
function CustomerFeedback() {
  const testimonials = [
    {
      ...
      quote: "Aireta tidak hanya memahami desain kami, tetapi juga karakter brand yang ingin kami bangun. Hasil produksinya rapi, komunikasinya jelas, dan setiap detail terasa sangat diperhatikan.",
    },
    {
      name: "Sarah Wijaya",
      ...
      quote: "Dari proses sampling sampai koleksi siap diluncurkan, tim Aireta sangat suportif. Mereka membantu kami menemukan solusi terbaik tanpa mengorbankan kualitas maupun visi kreatif.",
    },
    {
      name: "Alya Rahman",
      ...
      quote: "Kami merasa memiliki partner, bukan sekadar vendor produksi. Timeline terjaga, kualitas konsisten, dan hasil akhirnya bahkan melampaui ekspektasi tim kami.",
    },
    {
      name: "Catherine Lim",
      ...
      quote: "Professional, thoughtful, and remarkably detail-oriented. Aireta made the entire development process feel effortless and delivered a collection we are truly proud of.",
    },
  ];
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setActive((current) => (current + 1) % testimonials.length), 5500);
  ...
  return (
    <section className="feedback-section" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)}>
      <div className="mx-auto max-w-6xl px-5 py-14 sm:px-6 sm:py-20">
        <div className="mb-8 flex items-end justify-between gap-5 sm:mb-10">
          <div>
            ...
            <h2 className="mt-3 max-w-lg font-serif text-3xl leading-tight sm:text-4xl">Stories Behind Successful Collections</h2>
          </div>
          <div className="hidden gap-2 sm:flex">
            <button type="button" className="slider-arrow" aria-label="Previous testimonial" onClick={() => changeSlide(-1)}>
              ←
            </button>
            <button type="button" className="slider-arrow" aria-label="Next testimonial" onClick={() => changeSlide(1)}>
              →
            </button>
          </div>
        </div>
        ...
        <div className="feedback-slider">
          {testimonials.map((item, index) => (
            <article key={item.name} className={`feedback-slide ${index === active ? "feedback-slide-active" : ""}`} aria-hidden={index !== active}>
              <div className="feedback-image-wrap">
                <img src={item.image} alt={`Collection by ${item.role}`} />
                <span className="feedback-image-number">{String(index + 1).padStart(2, "0")}</span>
              </div>
        ...
        <div className="mt-6 flex items-center justify-between sm:justify-center">
          <div className="flex gap-2">
            {testimonials.map((item, index) => (
              <button key={item.name} type="button" className={`slider-dot ${index === active ? "slider-dot-active" : ""}`} aria-label={`Show testimonial ${index + 1}`} aria-current={index === active} onClick={() => setActive(index)} />
            ))}
          </div>
...
function ProductionJourney() {
  const steps = [
    ["Consultation", "Aligning your vision, requirements, and production goals."],
    ["Development", "Pattern development, material selection, and technical preparation"],
    ...
    ["Quality Assurance", "Detailed inspections throughout the production process."],
    ...
    <section className="journey-section">
      <div className="mx-auto max-w-6xl px-5 py-14 sm:px-6 sm:py-20">
        <div className="text-center">
          ...
          <h2 className="mt-3 font-serif text-3xl sm:text-4xl">From Sketch to Deliver</h2>
          <p className="mx-auto mt-3 max-w-xl text-sm leading-7 text-stone-500">An end-to-end manufacturing journey designed to bring your fashion vision to life.</p>
        </div>
    ...
    <section className="global-section">
      <div className="global-layout mx-auto max-w-6xl px-5 py-14 sm:px-6 sm:py-20">
        <div className="global-copy">
          ...
          <h2 className="mt-3 font-serif text-3xl leading-tight sm:text-4xl">Manufacturing for Modern Fashion Brands</h2>
          <p className="mt-5 text-sm leading-7 text-stone-600">From concept development to finished garments, we help brands bring their collections to life with precision, quality, and care.</p>
    ...
    <div>
      ...
      <section className="bg-white">
        <div className="max-w-6xl mx-auto px-6 py-10 grid md:grid-cols-2 gap-8 items-center">
          <img src={`${import.meta.env.BASE_URL}images/factory-landscape.jpg`} alt="factory" className="rounded" />
          <div>
            <p className="text-xs text-gray-400">ABOUT AIRETA</p>
            <h2 className="font-serif text-3xl mt-4">Built on Craftsmanship, Driven by Trust</h2>
            <p className="mt-4 text-gray-600">
              Aireta is a fashion manufacturing studio dedicated to helping brands bring their ideas to life. With a focus on quality, consistency, and attention to detail, we create an environment where creativity and craftsmanship work
              hand in hand. The result is a trusted partnership that supports brands in building collections with confidence.
            </p>
